...
export default {
  ...
  mounted() {
    ...
    axios
      .get('http://127.0.0.1:8000/api/client-reservation', {
        params: {
  ...
  methods:
  {
    ...
    confirmReservation(){
      axios.get(`http://127.0.0.1:8000/api/update-confirmation-client?reservation_id=${this.reservation_id}`)
        .then(response => {
    ...
    sendData() {
      ...
      axios.get(`http://127.0.0.1:8000/api/update-confirmation-code?code=${this.codeReserva}&branch_id=${this.branch_id}`)
        .then(response => {
          // Maneja la respuesta de la solicitud aquí
          this.codeConfirmation = response.data;
