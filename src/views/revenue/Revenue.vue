          ...
          <v-dialog v-model="dialogDelete" max-width="500px">
            <v-card>
              ...
              <v-card-text class="mt-2 mb-2">
                ¿Desea eliminar la operación de ingreso?</v-card-text
              >
              <v-divider></v-divider>
              <v-card-actions>
                <v-spacer></v-spacer>
       ...
       <v-icon size="25" color="red" @click="deleteItem(item)">
         mdi-delete
          ...
          <v-btn
            ...
            title="Editar operación de ingreso"
          ></v-btn>
          <v-btn
            density="comfortable"
            icon="mdi-delete"
            ...
            variant="tonal"
            elevation="1"
            title="Eliminar operación de ingreso"
          ></v-btn>
        </template>
      </v-data-table>
    </v-card-text>
...
(v) => /^[a-zA-ZáÁéÉíÍóÓúÚñÑ\s']+$/.test(v) || "El campo no es válido",*/
  ...
  methods: {
    ...
    deleteItemConfirm() {
      ...
      this.$axios.post("revenue-destroy", request)
        .then(() => {
          LocalStorageService.setIsLocked(false);
          this.initialize();
    ...
    save() {
      ...
      if (this.editedIndex > -1) {
        (this.valid = false), (this.data.id = this.editedItem.id);
        this.data.name = this.editedItem.name;
        this.$axios.put("revenue", this.data).then(() => {
          LocalStorageService.setIsLocked(false);
          this.initialize();
          this.showAlert("success", "Ingreso actualizado correctamente", 3000);
        });
      } else {
        (this.valid = false), (this.data.name = this.editedItem.name);
        this.$axios.post("revenue", this.data).then(() => {
          LocalStorageService.setIsLocked(false);
          this.initialize();
