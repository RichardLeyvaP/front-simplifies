                ...
                >
                  <template v-slot:item.name="{ item }">
                    <v-avatar elevation="3" color="grey-lighten-4" size="large">
                      <v-img
                        :src="`${this.$axios.defaults.baseURL}images/${item.image_service}`"
                        alt="image"
                      ></v-img>
                    </v-avatar>
                  </template>
...
export default {
  ...
  methods: {
    ...
    requestDelete() {
      ...
      this.$axios.post("order-destroy", request).then(() => {
        this.initialize();
    ...
    requestCancel(item) {
      ...
      this.$axios.put("order", request).then(() => {
        this.initialize();
        this.showDetails(this.reservation_resf);
        this.showAlert(
          "success",
    ...
    initialize() {
      this.$axios.get("branch-reservations", {
          params: {
    ...
    deleteItemConfirm() {
      ...
      console.log(request);
      this.$axios.post("reservation-destroy", this.data).then(() => {
        this.initialize();
    ...
    savePay() {
      {
        ...
        this.$axios.put("payment", this.data).then(() => {
    ...
    saveCloseBox() {
      ...
      this.$axios.post("closebox", this.data).then(() => {
    ...
    saveBox() {
      {
        ...
        this.$axios.put("box", this.data).then(() => {
