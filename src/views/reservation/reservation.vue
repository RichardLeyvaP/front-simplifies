    ...
    <v-toolbar color="#F18254">
      <v-row>
        <v-col cols="12" md="9" class="mt-4">
          <span class="ml-4">
            <strong>Reservaciones</strong></span>
        </v-col>
        ...
        <template v-slot:top>
          ...
          <v-dialog v-model="dialogRequest" width="500">
            <v-card>
              ...
              <v-card-actions>
                ...
                <v-btn color="warning" :loading="loading" @click="requestDelete"
                  >Aceptar</v-btn
                >
                <v-spacer></v-spacer>
              </v-card-actions>
          ...
          <v-dialog v-model="dialogDelete" max-width="600px">
            <v-card>
              ...
              <v-card-text class="mt-2 mb-2">
                ¿Desea eliminar la Reseervación Seleccionada?
                <v-text-field
                  v-model="cause"
                  clearable
                  label="Motico de la eliminación"
                ...
                <v-form v-model="valid" enctype="multipart/form-data">
                  <v-container>
                    ...
                    <v-row>
                      <v-col cols="12" md="4">
                        <v-text-field
                          v-model="editedItem.cardGif"
                          clearable
                          label="Tarjeta de Regalo"
                          prepend-icon="mdi-gift"
                          variant="underlined"
                          :rules="[pago, reglaValidacionDobleOpcional]"
                        >
                        </v-text-field>
                      </v-col>
                      <v-col cols="12" md="4">
                        <v-text-field
                          v-if="mostrarCode"
                          clearable
                          v-model="editedCard.cardGift_id"
                          label="Código"
                          prepend-icon="mdi-code"
                          variant="underlined"
                          @input="onCardGiftSelected"
                        ></v-text-field>
                      </v-col>
                      <v-col cols="12" md="4">
                        <v-text-field
                ...
                >
                  <template v-slot:item.name="{ item }">
                    <v-avatar elevation="3" color="grey-lighten-4" size="large">
                      <v-img
                        :src="
                          'https://api2.simplifies.cl/api/images/' + item.image_service
                        "
                        alt="image"
                      ></v-img>
                    </v-avatar>
...
export default {
  ...
  methods: {
    onCardGiftSelected(value) {
      ...
      axios
        .get("https://api2.simplifies.cl/api/show-value", {
          params: {
            code: value.data,
          },
        })
    ...
    requestDelete() {
      ...
      axios.post("https://api2.simplifies.cl/api/order-destroy", request).then(() => {
        this.initialize();
    ...
    requestCancel(item) {
      ...
      axios.put("https://api2.simplifies.cl/api/order", request).then(() => {
        this.initialize();
        this.showDetails(this.reservation_resf);
        this.showAlert(
          "success",
    ...
    initialize() {
      axios
        .get("https://api2.simplifies.cl/api/branch-reservations", {
          params: {
    ...
    showDetails(item) {
      this.reservation_resf = item;
      axios
        .get("https://api2.simplifies.cl/api/car_services", {
          params: {
    ...
    deleteItemConfirm() {
      ...
      axios
        .post("https://api2.simplifies.cl/api/reservation-destroy", this.data)
        .then(() => {
          this.initialize();
    ...
    savePay() {
      {
        ...
        this.data.creditCard = this.editedItem.creditCard
        ...
        axios.put("https://api2.simplifies.cl/api/payment", this.data).then(() => {
    ...
    saveCloseBox() {
      ...
      axios.post("https://api2.simplifies.cl/api/closebox", this.data).then(() => {
    ...
    saveBox() {
      {
        ...
        axios.put("https://api2.simplifies.cl/api/box", this.data).then(() => {
