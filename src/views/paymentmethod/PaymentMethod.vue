<template>
    <v-container fluid>
    <v-snackbar class="mt-12" location="right top" :timeout="sb_timeout" :color="sb_type" elevation="24"
        :multi-line="true" vertical v-model="snackbar">
        <v-row>
            <v-col md="2">
                <v-avatar :icon="sb_icon" color="sb_type" size="40"></v-avatar>
            </v-col>
            <v-col md="10">
                <h4>{{ sb_title }}</h4>
                {{ sb_message }}
            </v-col>
        </v-row>
    </v-snackbar>

    <v-card elevation="6" class="mx-5">
        <v-toolbar color="#F18254">
            <v-row align="center">
                <v-col cols="12" md="8" class="grow ml-4">
                    <span class="text-subtitle-1"><strong>Métodos de Pago</strong></span>
                </v-col>
                <v-col cols="12" md="3" class="text-right">
                    <v-btn class="text-subtitle-1 ml-12" color="#E7E9E9" variant="tonal" elevation="2"
                        prepend-icon="mdi-plus-circle" @click="showAdd">
                        Agregar Método
                    </v-btn>
                </v-col>
            </v-row>
        </v-toolbar>

        <v-card-text>
            <v-text-field class="mt-1 mb-1" v-model="search" append-icon="mdi-magnify" label="Buscar" single-line
                hide-details>
            </v-text-field>
            <v-data-table :headers="headers" :search="search" :items="paymentMethods" class="elevation-1"
                style="max-height: 68vh; overflow-y: auto;" :items-per-page-text="'Elementos por páginas'"
                no-data-text="No hay datos disponibles" :loading="loading" loading-text="Cargando datos...">
                <template v-slot:item.actions="{ item }">
                    <v-btn density="comfortable" icon="mdi-pencil" @click="editItem(item)" color="#1976D2"
                        variant="tonal" elevation="1" title="Editar Método"></v-btn>
                    <v-btn density="comfortable" icon="mdi-delete" @click="deleteItem(item)" :color="'red-darken-4'"
                        variant="tonal" elevation="1" title="Eliminar Método"></v-btn>
                </template>
                <template v-slot:item.type="{ item }">
                    <v-chip :color="getTypeColor(item.type)" dark>
                        {{ formatType(item.type) }}
                    </v-chip>
                </template>
            </v-data-table>
        </v-card-text>
    </v-card>

    <v-dialog v-model="dialog" max-width="600px">
        <v-form ref="form" v-model="valid">
            <v-card>
                <v-toolbar color="#F18254">
                    <span class="text-subtitle-2 ml-4">{{ formTitle }}</span>
                </v-toolbar>
                <v-card-text>
                    <v-container>
                        <v-row>
                            <v-col cols="12" md="6">
                                <v-text-field v-model="editedItem.name" label="Nombre"
                                    prepend-icon="mdi-cash-multiple" variant="underlined" :rules="nameRules"
                                    density="compact"></v-text-field>
                            </v-col>
                            <v-col cols="12" md="6">
                                    <v-select v-model="editedItem.type" :items="typeOptions" item-value="id" item-title="name" label="Tipo"
                  prepend-icon="mdi-format-list-checks" variant="underlined" density="compact">
                </v-select>
                            </v-col>
                            <v-col cols="12" md="12">
                                <v-textarea v-model="editedItem.description" label="Descripción"
                                    prepend-icon="mdi-note" variant="underlined" density="compact"></v-textarea>
                            </v-col>
                        </v-row>
                    </v-container>
                </v-card-text>
                <v-divider></v-divider>
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="#E7E9E9" variant="flat" @click="close">Cancelar</v-btn>
                    <v-btn color="#F18254" variant="flat" :loading="loading" @click="save"
                        :disabled="!valid">Aceptar</v-btn>
                </v-card-actions>
            </v-card>
        </v-form>
    </v-dialog>

    <v-dialog v-model="dialogDelete" max-width="500px">
        <v-card>
            <v-toolbar color="red">
                <span class="text-subtitle-2 ml-4"> Eliminar método de pago</span>
            </v-toolbar>
            <v-card-text class="mt-2 mb-2"> ¿Desea eliminar este método de pago?</v-card-text>
            <v-divider></v-divider>
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="#E7E9E9" variant="flat" @click="closeDelete">Cancelar</v-btn>
                <v-btn color="#F18254" variant="flat" :loading="loading" @click="deleteItemConfirm">Aceptar</v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
    </v-container>
</template>

<script>
import { handleRequest } from "@/utils/api";

export default {
    data: () => ({
        snackbar: false,
        sb_type: '',
        sb_message: '',
        sb_timeout: 2000,
        sb_title: '',
        sb_icon: '',
        valid: true,
        loading: false,
        dialog: false,
        dialogDelete: false,
        paymentMethods: [],
        data: {},

        headers: [
            { title: 'Nombre', value: 'name', width: '30%' },
            { title: 'Tipo', value: 'type', width: '30%' },
            { title: 'Descripción', value: 'description', width: '30%' },
            { title: 'Acciones', value: 'actions', sortable: false, width: '10%' },
        ],

        typeOptions: [
            { name: 'Efectivo', id: 'totalCash' },
            { name: 'Tarjeta de Crédito', id: 'totalCreditCard' },
            { name: 'Tarjeta de Débito', id: 'totalDebit' },
            { name: 'Transferencia', id: 'totalTransfer' },
            { name: 'Tarjeta de Regalo', id: 'totalCardGif' },
            { name: 'Otros medios', id: 'totalOther' },
        ],

        editedItem: {
            id: '',
            name: '',
            type: '',
            description: ''
        },
        defaultItem: {
            id: '',
            name: '',
            type: '',
            description: ''
        },
        editedIndex: -1,
        search: '',
        nameRules: [
            (v) => !!v || "El campo es requerido",
            (v) => (v && v.length <= 50) || "El campo debe tener menos de 51 caracteres",
            (v) => (v && v.length >= 3) || "El campo debe tener al menos de 3 caracteres",
        ],
        selectRules: [(v) => !!v || "Seleccionar al menos un elemento"],
    }),
    computed: {
        formTitle() {
            return this.editedIndex === -1 ? 'Agregar Nuevo Método' : 'Editar Método';
        }
    },
    mounted() {
        this.initialize();
    },
    methods: {
        showAdd() {
            this.dialog = true;
        },
        close() {
            this.dialog = false;
            this.$nextTick(() => {
                this.editedItem = Object.assign({}, this.defaultItem);
            });
            this.editedIndex = -1;
        },
        async initialize() {
            try {
                this.loading = true;
                const result = await handleRequest({
                    endpoint: 'payment-method',
                    method: 'GET',
                });

                if (result.success) {
                    this.paymentMethods = result.data.paymentOptions || [];
                } else {
                    this.paymentMethods = [];
                }
            } catch (error) {
                this.loading = false;
                this.showAlert('error', 'Ocurrió un error inesperado al cargar los métodos de pago.', 3000);
            } finally {
                this.loading = false;
            }
        },
        formatType(type) {
            const option = this.typeOptions.find(item => item.value === type);
            return option ? option.text : type;
        },
        getTypeColor(type) {
            const colors = {
                'totalCash': 'green',
                'totalCreditCard': 'blue',
                'totalDebit': 'blue-darken-2',
                'totalTransfer': 'purple',
                'totalCardGif': 'orange',
                'totalOther': 'grey'
            };
            return colors[type] || 'grey';
        },
        async save() {
            this.loading = true;
            if (this.editedIndex === -1) {
                this.data = {};
                
                this.data.name = this.editedItem.name;
                this.data.type = this.editedItem.type;
                this.data.description = this.editedItem.description;
                try {
                    const result = await handleRequest({
                        endpoint: 'payment-method',
                        method: 'POST',
                        data: this.data
                    });

                    if (result.success) {
                        this.showAlert("success", result.message, 3000);
                        this.initialize();
                    } else {
                        this.showAlert("warning", result.message, 3000);
                    }
                } catch (error) {
                    this.showAlert("error", "Ocurrió un error inesperado al procesar la solicitud.", 3000);
                } finally {
                    this.loading = false;
                }
            } else {
                
                try {
                    const params = {
                        id: this.edieditedItem.id,
                        name: this.editedItem.name,
                        type: this.editedItem.type,
                        description: this.editedItem.description
                    };
                    const result = await handleRequest({
                        endpoint: 'payment-method',
                        method: 'PUT',
                        params: params
                    });

                    if (result.success) {
                        this.showAlert("success", result.message, 3000);
                        this.initialize();
                    } else {
                        this.showAlert("warning", result.message, 3000);
                    }
                } catch (error) {
                    this.showAlert("error", "Ocurrió un error inesperado al procesar la solicitud.", 3000);
                } finally {
                    this.loading = false;
                    this.editedIndex = -1;
                }
            }
            this.close();
        },
        editItem(item) {
            this.editedIndex = 1;
            this.editedItem = Object.assign({}, item);
            this.dialog = true;
        },
        deleteItem(item) {
            this.editedIndex = 1;
            this.editedItem.id = item.id;
            this.dialogDelete = true;
        },
        closeDelete() {
            this.dialogDelete = false
            this.$nextTick(() => {
                this.editedItem = Object.assign({}, this.defaultItem)
            })
        },
        async deleteItemConfirm() {
            this.loading = true;
            try {
                const params = {
                        id: this.edieditedItem.id,
                    };
                const result = await handleRequest({
                    endpoint: `payment-method-destroy`,
                    method: 'POST',
                    params: params
                });

                if (result.success) {
                    this.showAlert("success", result.message, 3000);
                    this.initialize();
                } else {
                    this.showAlert("warning", result.message, 3000);
                }
            } catch (error) {
                this.showAlert("error", "Ocurrió un error inesperado al procesar la solicitud.", 3000);
            } finally {
                this.loading = false;
                this.closeDelete();
            }
        },
        showAlert(sb_type, sb_message, sb_timeout) {
            this.sb_type = sb_type;

            if (sb_type == "success") {
                this.sb_title = "Éxito";
                this.sb_icon = "mdi-check-circle";
            }

            if (sb_type == "error") {
                this.sb_title = "Error";
                this.sb_icon = "mdi-alert-circle";
            }

            if (sb_type == "warning") {
                this.sb_title = "Advertencia";
                this.sb_icon = "mdi-alert-circle";
            }
            this.sb_message = sb_message;
            this.sb_timeout = sb_timeout;
            this.snackbar = true;
        },
    },
};
</script>