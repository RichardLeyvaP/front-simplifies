        ...
        <v-row align="center">
          <v-col cols="12" md="4" class="grow ml-4">
            <span class="text-subtitle-1">
              <strong>Horarios de Trabajo por Sucursales</strong></span
            >
          </v-col>
                    ...
                    <v-row align="center" no-gutters>
                      <v-col cols="2">
                        <v-select
                          variant="underlined"
                          :items="horas"
                          v-model="dia.entradaHora"
                          dense
                          outlined
                        ></v-select>
                      </v-col>
                      <v-col cols="1" class="pl-2"> : </v-col>
                      <v-col cols="2">
                        <v-select
                          variant="underlined"
                          :items="minutos"
                          v-model="dia.entradaMinuto"
                          dense
                          outlined
                        ></v-select>
                      </v-col>
                    ...
                    <v-row no-gutters>
                      <v-col cols="2">
                        <v-select
                          variant="underlined"
                          :items="horas"
                          v-model="dia.salidaHora"
                          dense
                          outlined
                        ></v-select>
                      </v-col>
                      <v-col cols="1" class="mt-5 pl-2"> : </v-col>
                      <v-col cols="2">
                        <v-select
                          variant="underlined"
                          :items="minutos"
                          v-model="dia.salidaMinuto"
                          dense
                          outlined
                        ></v-select>
                      </v-col>
...
export default {
  ...
  methods: {
    ...
    initialize() {
      LocalStorageService.setIsLocked(true);
      this.$axios.get("show_schedule_branch", {
          params: {
        ...
        .finally(() => {
          ...
          this.dias = this.results.Schedules.map((schedule) => ({
            ...
            entradaHora:
              schedule.start_time !== null ? schedule.start_time.split(":")[0] : "00",
            entradaMinuto:
              schedule.start_time !== null ? schedule.start_time.split(":")[1] : "00",
            salidaHora:
              schedule.closing_time !== null ? schedule.closing_time.split(":")[0] : "00",
            salidaMinuto:
              schedule.closing_time !== null ? schedule.closing_time.split(":")[1] : "00",
          }));
        });
    },
    ...
    save() {
      ...
      const updatedData = this.dias.map((dia) => {
        ...
        //console.log(dia.salidaMinuto);
        const start_time =
          dia.entradaHora || dia.entradaMinuto
            ? `${dia.entradaHora}:${dia.entradaMinuto}`
            : null;
        const closing_time =
          dia.salidaHora || dia.salidaMinuto
            ? `${dia.salidaHora}:${dia.salidaMinuto}`
            : null;
        return {
          day: dia.nombre,
          start_time:
            dia.esLaboral === false ? null : start_time === "00:00" ? null : start_time,
          closing_time:
            dia.esLaboral === false
              ? null
              : closing_time === "00:00"
      ...
      console.log("request");
      console.log(request);
      this.$axios.put("schedule", request)
        .then(() => {
          LocalStorageService.setIsLocked(false);
          this.initialize();
