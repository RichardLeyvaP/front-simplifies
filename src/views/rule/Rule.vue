                    ...
                    <v-row>
                      <v-col cols="12" md="12">
                        <v-text-field
                          ...
                          clearable
                          label="Nombre de la Regla de convivencia"
                    ...
                    <v-row>
                      <v-col cols="12" md="12">
                        <v-text-field
                          ...
                          clearable
                          label="Tipo de Regla de convivencia"
                          prepend-icon="mdi-ruler"
                          variant="underlined"
                          :rules="nameRules"
                          :disabled="editedItem.automatic === 1"
                        >
                        </v-text-field>
                      </v-col>
                      <!--<v-col cols="12" md="4">
          ...
          <v-dialog v-model="dialogDelete" max-width="500px">
            <v-card>
              ...
              <v-card-text class="mt-2 mb-2">
                ¿Desea eliminar la regla de convivencia?</v-card-text
              >
              <v-divider></v-divider>
              <v-card-actions>
                <v-spacer></v-spacer>
      ...
      >
        <template v-slot:item.automatic="{ item }">
          <div class="d-flex justify-content-center">
            <v-chip
              :color="item.automatic === '1' ? 'green' : 'red'"
              :text="item.automatic === '1' ? 'Si ' : 'No'"
        ...
        <template v-slot:item.actions="{ item }">
          <v-btn
            ...
            title="Editar Regla de convivencia"
          ></v-btn>
          <v-btn
            density="comfortable"
            icon="mdi-delete"
            @click="item.automatic !== '1' && deleteItem(item)"
            :color="item.automatic === '1' ? 'grey' : 'red'"
            variant="tonal"
            elevation="1"
            title="Eliminar Regla de convivencia"
          ></v-btn>
        </template>
      </v-data-table>
    </v-card-text>
...
export default {
  data: () => ({
    ...
    nameRules: [
      ...
      (v) => (v && v.length <= 40) || "El campo debe tener menos de 41 caracteres",
  ...
  methods: {
    ...
    deleteItemConfirm() {
     ...
     this.$axios.post("rule-destroy", request)
        .then(() => {
          LocalStorageService.setIsLocked(false);
          this.initialize();
    ...
    save() {
      ...
      if (this.editedIndex > -1) {
        ...
        console.log(this.editedItem.automatic);
        this.$axios.put("rule", this.data).then(() => {
          LocalStorageService.setIsLocked(false);
          this.initialize();
        ...
        //this.data.automatic = this.editedItem.automatic;
        console.log(this.editedItem.automatic);
        this.$axios.post("rule", this.data).then(() => {
          LocalStorageService.setIsLocked(false);
          this.initialize();
          this.showAlert(
            "success",
            "Regla de convivencia registrada correctamente",
            3000
          );
        });
      }
