        ...
        >
          <template v-slot:item.name="{ item }">
            <v-avatar class="mr-5" elevation="3" color="grey-lighten-4">
              <v-img
                :src="`${$axios.defaults.baseURL}images/${item.image_data}`"
                alt="image"
              ></v-img>
            </v-avatar>
            {{ item.name }}
...
export default {
  ...
  methods: {
    ...
    initialize() {
      this.loading = true;
      LocalStorageService.setIsLocked(true);
      this.$axios.get("enrollment-show", { params: { business_id: this.business_id } })
        .then((response) => {
    ...
    deleteItemConfirm() {
     ...
     this.$axios.post("enrollment-destroy", request)
        .then(() => {})
        .finally(() => {
          LocalStorageService.setIsLocked(false);
          this.initialize();
    ...
    save() {
      ...
      if (this.editedIndex == 2) {
        ...
        console.log(formData);
        this.$axios.post("enrollment-updated", formData)
          .then(() => {
            this.file = null;
            this.imgMiniatura = "";
          })
      ...
      if (this.editedIndex == 1) {
       ...
       this.$axios.post("enrollment", formData)
          .then(() => {
            this.file = null;
            this.imgMiniatura = "";
          })
    ...
    showAddStores() {
      LocalStorageService.setIsLocked(true);
      this.$axios
        .get("enrollmentstore-show-notIn", {
          params: {
    ...
    saveStore() {
      ...
      console.log(this.data);
      this.$axios
        .post("enrollmentstore", this.data)
        .then(() => {
          this.dialogAddStore = false;
          this.store_id = "";
        })
    ...
    storeDelete() {
      ...
      this.$axios
        .post("enrollmentstore-destroy", request)
        .then(() => {
          this.dialogRequestStore = false;
          this.store_id = "";
