...
<template>
          ...
          <v-col cols="12" md="4">
            <v-card class="mx-auto" max-width="400">
              <v-img
                ...
                height="300"
                :src="`${this.$axios.defaults.baseURL}images/${history.imageLook}`"
                cover
                v-on:click="openImageDialog(`${this.$axios.defaults.baseURL}images/${history.imageLook}`)"
              >
                <v-card-title>
                ...
                <v-row>
                  <v-col cols="12" md="12">
                    <div class="text-h8 mt-3">
                      ...
                      >
                        <v-img
                          :src="`${this.$axios.defaults.baseURL}images/${history.image_url}`"
                          alt="Imagen"
                        ></v-img>
                      </v-avatar>
              ...
              <v-list lines="two" item-props>
                ...
                >
                  <template v-slot:prepend>
                    <v-avatar class="mr-5" elevation="3" color="grey-lighten-4">
                      <v-img
                        :src="`${this.$axios.defaults.baseURL}images/${product.image_product}`"
                        alt="image"
                      ></v-img>
                    </v-avatar>
                  </template>
  ...
  <v-card-text>
    ...
    <v-card-title class="pa-0">
                  ...
                  <v-col cols="12" md="6">
                    <v-autocomplete :no-data-text="'No hay datos disponibles'" v-model="editedCardGiftUser.card_gift_id" :items="cardGiftsData" label="Tarjeta de Regalo"
                      ...
                      <v-list-item v-bind="props"
                        :prepend-avatar="`${this.$axios.defaults.baseURL}images/${item.raw.image_cardgift}`"
                        :title="item.raw.name">
      ...
      <!--ClientFrecuence-->
    <v-dialog v-model="dialogSurvey" fullscreen transition="dialog-bottom-transition">
      <v-card>
        <v-toolbar color="#F18254">
      <v-container fluid>
        <v-row align="center">
          ...
          <v-col cols="12" md="3">
            <v-btn class="text-subtitle-1  ml-12" color="#E7E9E9" variant="flat" elevation="2"
              prepend-icon="mdi-file-excel" @click="exportToExcelSurvey">
              Exportar a Excel
            </v-btn>
          </v-col>
        </v-row>
    ...
    <v-container fluid>
      <v-row>
        ...
        <v-col cols="12" md="12">
          ...
          <v-card-text>
            <v-text-field class="mt-1 mb-1" v-model="searchSurvey" append-icon="mdi-magnify" label="Buscar" single-line
              hide-details></v-text-field>
            <v-data-table :headers="headersSurvey" :items-per-page-text="'Elementos por páginas'" :items="survey"
              :search="searchSurvey" class="elevation-2" no-results-text="No hay datos disponibles"
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="#E7E9E9" variant="flat" @click="closeSurvey"> Volver </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-card>
...
export default {
    ...
    this.$axios.get('show-business', {
      ...
      .then((response) => {
        ...
        if (this.charge === 'Administrador') {
          ...
          this.branch_idSurvey = this.branches[0].id;
      this.mostrarFila = true;
    }else{
      this.branch_idSurvey = this.branch_id;
    } 
    this.initialize();
          });
  },
  ...
  methods: {
    ...
    editItem(item) {
      this.file = null;
      var img = new Image();
      const imagUrl = `${this.$axios.defaults.baseURL}images/${item.client_image}`
      img.src = imagUrl;
      img.onload = () => {
    ...
    deleteItemConfirm() {
      ...
      this.$axios.post("client-destroy", request)
        .then(() => {})
        .finally(() => {
          LocalStorageService.setIsLocked(false);
    ...
    save() {
      ...
      if (this.editedIndex > -1) {
        ...
        this.$axios.post("client-update", formData)
          .then(() => {})
          .finally(() => {
            LocalStorageService.setIsLocked(false);
    ...
    ClientsFrecuence() {
      ...
      const startDate = this.input ? format(this.input, "yyyy-MM-dd") : format(new Date(), "yyyy-MM-dd");
      const endDate = this.input2 ? format(this.input2, "yyyy-MM-dd") : format(new Date(), "yyyy-MM-dd");
      this.$axios.get('clients-frecuence-periodo', {
          params: {
            branch_id: this.branch_id,
            startDate: startDate,
            endDate: endDate
    ...
    showAddCardGiftClient(){
      this.details = '';
      LocalStorageService.setIsLocked(true);
      this.$axios.get('client-autocomplete1', {
                    params: {
                        branch_id: this.branch_id,
                    }
                })
    ...
    showUserCardGift(item) {
      ...
      LocalStorageService.setIsLocked(true);
      this.$axios.get('card-gift-user-cardgift', {
          params: {
    ...
    exportToExcelSurvey() {
      ...
      // Construye un objeto para los encabezados basado en la estructura de 'headers'
      let headerRow = {};
      this.headersSurvey.forEach(header => {
        headerRow[header.key] = header.title; // Usa 'key' para el mapeo y 'title' para el texto del encabezado
      });
      rows.push(headerRow);
